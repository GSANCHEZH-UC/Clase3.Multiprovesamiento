...
sales_by_category = df_completo.groupby('category')['revenue'].sum().reset_index()
sales_by_gender = df_completo.groupby('gender')['revenue'].sum().reset_index()
sales_by_city = df_completo.groupby('city')['revenue'].sum().reset_index()
...
print(f"Tiempo de análisis: {fin_analisis - inicio_analisis:.5f} segundos")
input("Presiona Enter para continuar...")
